...
describe('Component: LoginLocations', () => {
  ...
  // Выше я сделал тест проверки работоспособности кнопки открытия меню сортировки.
  // Но проверял я не факт нажатия кнопки, а факт добавления/удаления нужного класса
  // Не смог понять как протестировать функцию, которая объявляется непосредственно в компоненте, а не приходит в пропсах или вызывает dispatch
  // Вызов этой же функции, переданной в пропсе в дочерний компонент places-sorting-option прекрасно тестируется там. А код ниже не проходит проверку
